...
pub fn test_byteslice_from_charrange_normal() {
    ...
    assert_eq!(
        get_byteslice_from_charrange(s, 1..3),
    ...
    assert_eq!(
        get_byteslice_from_charrange(s, 5..),
        [
    ...
    assert_eq!(
        get_byteslice_from_charrange(s, ..=2),
...
pub fn test_byteslice_from_startcharcount() {
    let s = "Aé♫山𝄞🐗🐻‍❄️";
    assert_eq!(
        get_byteslice_from_startcharcount(s, 4),
...
pub fn test_byteslice_from_endcharcount() {
    let s = "Aé♫山𝄞🐗🐻‍❄️";
    assert_eq!(
        get_byteslice_from_endcharcount(s, 3),
        [0xE2, 0x80, 0x8D, 0xE2, 0x9D, 0x84, 0xEF, 0xB8, 0x8F]
    );
}
...
pub fn test_bytevector_from_charrange() {
    ...
    assert_eq!(
        get_bytevector_from_charrange(s, 5..),
        vec![
            0xF0, 0x9F, 0x90, 0x97, 0xF0, 0x9F, 0x90, 0xBB, 0xE2, 0x80, 0x8D, 0xE2, 0x9D, 0x84,
            0xEF, 0xB8, 0x8F,
        ]
    ...
    assert_eq!(
        get_bytevector_from_charrange(s, ..2),
        vec![0x41, 0xC3, 0xA9]
    );
    assert_eq!(
        get_bytevector_from_charrange(s, ..=2),
...
pub fn test_charvector_from_charrange() {
    ...
    assert_eq!(
        get_charvector_from_charrange(s, ..),
        vec![
            'A', 'é', '♫', '山', '𝄞', '🐗', '🐻', '\u{200D}', '❄', '\u{FE0F}'
        ]
    );
    ...
    assert_eq!(
        get_charvector_from_charrange(s, 5..),
        vec!['🐗', '🐻', '\u{200D}', '❄', '\u{FE0F}']
    );
...
#[test]
#[should_panic(
    expected = "Char range start 6 is not a glyph boundary; it is inside glyph 'é' (characters 5..7, bytes 17..20)"
)]
...
#[test]
#[should_panic(
    expected = "Char range end 9 is not a glyph boundary; it is inside glyph '🐻‍❄️' (characters 7..11, bytes 20..33)"
)]
...
#[test]
#[should_panic(
    expected = "Char range start 3 is not a glyph boundary; it is inside glyph '🐻‍❄️' (characters 0..4, bytes 0..13)"
)]
