...
pub fn drive_type(path: &Path) -> Result<DriveType, String> {
    ...
    let root_path = match path_abs.components().next() {
        Some(std::path::Component::Prefix(prefix)) => match prefix.kind() {
            ...
            std::path::Prefix::UNC(server, share) => &format!("\\\\{}\\{}\\", server.to_string_lossy(), share.to_string_lossy()),
            _ => return Err("Path is not a drive or UNC path".to_string()),
        },
        _ => return Err("Path does not have a drive component".to_string()),
    };
    ...
    unsafe {
        ...
        if resu32 <= 6 {
            Ok(std::mem::transmute::<u32, DriveType>(resu32))
        } else {
