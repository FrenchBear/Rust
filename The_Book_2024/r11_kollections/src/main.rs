...
fn mode(v: &Vec<i32>) -> ModeResult {
    if v.len()==0 {return ModeResult::Undefined};   // v is empty
    ...
    if vc.len()==1 {return ModeResult::SingleMode(v[0]);}       // v contains a single value (must be checked before following test)
    if *vc[0].1==1 {return ModeResult::Undefined;}               // Max count=1 and more than 1 different value, mode is not defined
    if vc[1].1==vc[0].1 {return ModeResult::MultiMode;}         // If the first two max counts are identical, it's a multimode series
...
fn pig_latin(sentence: &str) {
    ...
    for word in sentence.split(" ") {
        let c1 = word.chars().nth(0).unwrap().to_ascii_lowercase();
